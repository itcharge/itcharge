...
![iOS Runtime 02 001](https://qcdn.itcharge.cn/images/iOS-Runtime-02-001.png)
...
![iOS Runtime 02 002](https://qcdn.itcharge.cn/images/iOS-Runtime-02-002.png)
...
- (void)checkEmpty {
    ...
    if ([dataSource respondsToSelector:@selector(numberOfSectionsInTableView:)]) {
        sections = [dataSource numberOfSectionsInTableView:self] - 1; // 获取当前 TableView 组数
    }

    for (NSInteger i = 0; i <= sections; i++) {
        NSInteger rows = [dataSource tableView:self numberOfRowsInSection:i]; // 获取当前 TableView 各组行数
        if (rows) {
